<script lang="ts">
  ...
  $: {
    if (tourData.itinerary) {
      ...
      if (itineraryImageFiles.length !== itineraryLength) {
        ...
        mealsImagePreviews = tourData.itinerary.map(day => 
          (day.meals || []).map(() => [])
        );
      }
    }
  ...
  function addMeal(dayIndex: number) {
    ...
    tourData.itinerary[dayIndex].meals = [...(tourData.itinerary[dayIndex].meals || []), {
      name: '',
      description: '',
      images: []
    }];
  ...
  function removeMeal(dayIndex: number, mealIndex: number) {
    if (tourData.itinerary[dayIndex].meals) {
      tourData.itinerary[dayIndex].meals = tourData.itinerary[dayIndex].meals!.filter((_, i) => i !== mealIndex);
    }
  }
  ...
  function handleAccommodationImageChange(event: Event, dayIndex: number) {
    ...
    newFiles.forEach(file => {
      const reader = new FileReader();
      reader.onload = (e) => {
        accommodationImagePreviews[dayIndex] = [...accommodationImagePreviews[dayIndex], e.target?.result as string];
  ...
  function removeAccommodationImage(dayIndex: number, imageIndex: number) {
    accommodationImageFiles[dayIndex] = accommodationImageFiles[dayIndex].filter((_, i) => i !== imageIndex);
    accommodationImagePreviews[dayIndex] = accommodationImagePreviews[dayIndex].filter((_, i) => i !== imageIndex);
    accommodationImageFiles = [...accommodationImageFiles];
    accommodationImagePreviews = [...accommodationImagePreviews];
  }
  ...
  function handleMealImageChange(event: Event, dayIndex: number, mealIndex: number) {
    ...
    newFiles.forEach(file => {
      const reader = new FileReader();
      reader.onload = (e) => {
        mealsImagePreviews[dayIndex][mealIndex] = [...mealsImagePreviews[dayIndex][mealIndex], e.target?.result as string];
  ...
  function removeMealImage(dayIndex: number, mealIndex: number, imageIndex: number) {
    mealsImageFiles[dayIndex][mealIndex] = mealsImageFiles[dayIndex][mealIndex].filter((_, i) => i !== imageIndex);
    mealsImagePreviews[dayIndex][mealIndex] = mealsImagePreviews[dayIndex][mealIndex].filter((_, i) => i !== imageIndex);
    mealsImageFiles = [...mealsImageFiles];
    mealsImagePreviews = [...mealsImagePreviews];
  }
...
<form on:submit|preventDefault={handleSubmit} class="space-y-8">
        ...
        <div class="p-4 border border-neutral-200 rounded-lg">
          ...
          <div class="form-group">
            ...
            <div class="mt-2">
              ...
              >
                <svg class="h-4 w-4 mr-2 text-neutral-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                {mode === 'edit' ? $t('agent.tours.change_day_image') : $t('agent.tours.select_day_image')}
              </label>
            </div>
          ...
          <div class="form-group mt-6">
            ...
            <div class="space-y-3">
              ...
              <div>
                <label class="form-label text-sm">{$t('agent.tours.accommodation.description')}</label>
                <div class="flex gap-2">
                  <textarea 
                      ...
                      {#each day.accommodation.images as existingImage, imgIndex}
                        {#if !removedAccommodationImages.some(removed => removed.dayIndex === index && removed.imageIndex === imgIndex)}
                          <div class="relative aspect-[4/3] w-full">
                            ...
                            <div class="absolute bottom-1 left-1 px-2 py-1 bg-blue-500 text-white text-xs rounded">
                              Existing
                            </div>
                          </div>
                        {/if}
            ...
            {#each day.meals || [] as meal, mealIndex}
              <div class="border border-neutral-100 rounded p-3 mb-3">
                <div class="flex justify-between items-center mb-2">
                  <span class="text-sm font-medium">{$t('agent.tours.meals.meal_number')} {mealIndex + 1}</span>
                  <button 
                    type="button"
                ...
                <div class="space-y-2">
                  ...
                  <div>
                    ...
                    <div class="mt-1">
                      ...
                      >
                        <svg class="h-3 w-3 mr-1 text-neutral-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                            ...
                            {#if !removedMealImages.some(removed => removed.dayIndex === index && removed.mealIndex === mealIndex && removed.imageIndex === imgIndex)}
                              <div class="relative aspect-[4/3] w-full">
                                ...
                                <button 
                                  type="button"
                                  class="absolute top-0.5 right-0.5 p-0.5 bg-red-500 text-white rounded-full text-xs"
                                  on:click={() => removeExistingMealImage(index, mealIndex, imgIndex)}
                                >
                                  <svg class="h-2 w-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                  </svg>
                                </button>
                                <div class="absolute bottom-0 left-0 px-1 py-0.5 bg-blue-500 text-white text-xs rounded-tr">
                                  Existing
                                </div>
                              </div>
                            {/if}
  ...
  <div class="bg-white rounded-lg shadow-sm p-6 border border-neutral-200">
    ...
    <div class="form-group">
      ...
      <div class="mt-2">
        ...
        >
          ...
          {mode === 'edit' ? $t('agent.tours.images.change_main_image') : $t('agent.tours.select_image')}
        </label>
      </div>
      {#if mainImagePreview}
        <div class="mt-4 w-64">
    ...
    <div class="form-group mt-6">
      <label for="gallery-images" class="form-label">{$t('agent.tours.images.gallery_images')}</label>
      <div class="mt-2">
        <input 
          type="file" 
          id="gallery-images" 
