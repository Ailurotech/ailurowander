...
<div class="relative inline-block">
  ...
  <button
    ...
    class="inline-flex items-center gap-1.5 rounded-md font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed {sizeClasses[
      size
  ...
  {#if error}
    <div
      class="absolute top-full left-0 mt-1 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700 z-10"
    >
      {error}
    </div>
  {/if}
  ...
  {#if showResults && translationResult}
    <div
      class="absolute top-full left-0 mt-2 p-4 bg-white border border-gray-200 rounded-lg shadow-lg z-20 min-w-80"
    >
      <div class="flex justify-between items-center mb-3">
        <h4 class="font-medium text-gray-900">Translation Results</h4>
        <button type="button" on:click={closeResults} class="text-gray-400 hover:text-gray-600">
          <svg
            class="h-4 w-4"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
