<script lang="ts">
  ...
  $: if (tour.itinerary) {
    tour.itinerary.forEach((day, index) => {
      if (day.accommodation && day.accommodation.name && expandedAccommodation[index] === undefined) {
            ...
            <div class="flex-1">
                ...
                <div class="mt-4">
                  ...
                  >
                    <div class="flex items-center">
                      ...
                      <span class="font-medium">{$t('tour_detail.accommodation')}: {day.accommodation.name}</span>
                    </div>
                    <svg 
                      class="h-4 w-4 transition-transform duration-200 {expandedAccommodation[index] ? 'rotate-90' : ''}" 
                  ...
                  <div 
                    class="overflow-hidden transition-all duration-300 ease-in-out {expandedAccommodation[index] ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}"
                  >
                    <div class="mt-3 pl-6">
              ...
              {#if day.meals && day.meals.length > 0}
                <div class="mt-4">
                  ...
                  >
                    <div class="flex items-center">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 text-orange-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <span class="font-medium">{$t('tour_detail.meals')} ({day.meals.length})</span>
                    </div>
                    <svg 
                      class="h-4 w-4 transition-transform duration-200 {expandedMeals[index] ? 'rotate-90' : ''}" 
                      xmlns="http://www.w3.org/2000/svg" 
                      fill="none" 
                      viewBox="0 0 24 24" 
                      stroke="currentColor"
                  ...
                  <div 
                    class="overflow-hidden transition-all duration-300 ease-in-out {expandedMeals[index] ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}"
                  >
