<script lang="ts">
  ...
  export let backgroundImage: string = '/images/hero.jpg';
    ...
    <div class="max-w-4xl mx-auto px-4">
      ...
      {#if primaryCta || secondaryCta}
        <div class="flex flex-wrap gap-4 justify-center">
          {#if primaryCta}
            <a href={primaryCta.href} class="btn btn-primary hover:scale-105 transition-transform duration-300">{primaryCta.text}</a>
          {/if}
          {#if secondaryCta}
            <a href={secondaryCta.href} class="btn btn-outline btn-secondary text-white hover:text-white hover:scale-105 transition-transform duration-300">{secondaryCta.text}</a>
          {/if}
        </div>
      {/if}
    </div>
