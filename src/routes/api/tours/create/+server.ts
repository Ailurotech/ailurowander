...
import { createTour, uploadTourImages, updateTourImages, uploadItineraryImages, updateTourItineraryImages, uploadAccommodationImages, uploadMealsImages, updateTourAccommodationAndMealsImages } from '$lib/server/services/tourService';
...
export async function POST({ request }: RequestEvent) {
  ...
  try {
    ...
    if (hasAccommodationImages || hasMealsImages) {
      const accommodationImageUrls = hasAccommodationImages ? await uploadAccommodationImages(accommodationImages, tour._id.toString()) : [];
      const mealsImageUrls = hasMealsImages ? await uploadMealsImages(mealsImages, tour._id.toString()) : [];
      await updateTourAccommodationAndMealsImages(tour._id.toString(), itinerary, accommodationImageUrls, mealsImageUrls);
    }
